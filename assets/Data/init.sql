...
CREATE VIEW ALL_TRANS AS
    SELECT B.ID,A.ID,T.ID,T.DATE,T.AMOUNT FROM TRANSACTION_HISORY T JOIN ACCOUNT A ON A.ID =T.ACCOUNTID JOIN ASSOCIATED ON  A.ID = ASSOCIATED.TYPE JOIN BANK_ACCOUNT AS B ON ASSOCIATED.BANK_ACCOUNTID = B.ID;