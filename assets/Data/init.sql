...
CREATE TABLE IF NOT EXISTS TRANSACTION_MODIFIED(
    ...
    TELLERID INTEGER,
    DATE DATE,
    FOREIGN KEY (TELLERID) REFERENCES TELLER(ID) ON DELETE CASCADE,
    FOREIGN KEY (TRANSID) REFERENCES TRANSACTION_HISORY(ID)
);
...
INSERT INTO TRANSACTION_HISORY(ACCOUNTID,AMOUNT,DATE,TYPE) 
    SELECT ID,AMOUNT,DATE,3 FROM ACCOUNT; 
...
CREATE VIEW BANK_DATA AS 
    SELECT B.ID, A.AMOUNT,A.DATE,T.DESCRIPTION FROM ASSOCIATED JOIN BANK_ACCOUNT B ON B.ID = ASSOCIATED.BANK_ACCOUNTID JOIN ACCOUNT A ON ASSOCIATED.TYPE = A.ID JOIN ACCOUNT_TYPE T ON A.TYPE = T.ID;
-- Trasation associated with bankAccounts and accounttypes
CREATE VIEW ALL_TRANS AS
    SELECT B.ID AS B_ID,A.ID AS A_ID,T.ID,T.DATE,T.AMOUNT,TT.DESCRIPTION FROM TRANSACTION_HISORY T JOIN ACCOUNT A ON A.ID =T.ACCOUNTID JOIN ASSOCIATED ON  A.ID = ASSOCIATED.TYPE JOIN BANK_ACCOUNT AS B ON ASSOCIATED.BANK_ACCOUNTID = B.ID JOIN TRANSACTION_TYPE TT ON T.TYPE = TT.ID;
-- indepentent TRIGGERS
--UDATES ACCOUNT WITH TRASATIONS 
CREATE TRIGGER IF NOT EXISTS UDPATE_ACCOUNT 
    AFTER  INSERT ON TRANSACTION_HISORY
    FOR EACH ROW
        BEGIN
            UPDATE ACCOUNT
            SET  AMMOUNT = AMMOUNT + NEW.AMOUNT
            WHERE AMOUNT.ID = NEW.ACCOUNTID;
        END;
    ...
    AFTER INSERT ON TRANSACTION_MODIFIED
        FOR EACH ROW
            BEGIN
                ...
                INSERT INTO TRANSACTION_HISTORY (ACCOUNTID, AMOUNT, DATE, TYPE)
                VALUES (NEW.ACCOUNTID, -NEW.AMOUNT, NEW.DATE, 4);
            END;
