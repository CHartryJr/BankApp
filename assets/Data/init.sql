...
INSERT INTO TRANSACTION_HISORY(ACCOUNTID,AMOUNT,DATE,TYPE,EFFECTED_ACC) 
...
CREATE VIEW ALL_TRANS AS
    SELECT B.ID AS B_ID,A.ID AS A_ID,T.ID,T.DATE,T.AMOUNT,TT.DESCRIPTION,T.EFFECTED_ACC FROM TRANSACTION_HISORY T JOIN ACCOUNT A ON A.ID =T.ACCOUNTID JOIN ASSOCIATED ON  A.ID = ASSOCIATED.TYPE JOIN BANK_ACCOUNT AS B ON ASSOCIATED.BANK_ACCOUNTID = B.ID JOIN TRANSACTION_TYPE TT ON T.TYPE = TT.ID;
    ...
    AFTER INSERT ON TRANSACTION_MODIFIED
        FOR EACH ROW
            BEGIN
                ...
                VALUES (NEW.TRANSID, -NEW.AMOUNT, NEW.DATE, 4,NEW.TRANSID);
            END;