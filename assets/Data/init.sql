...
CREATE TABLE IF NOT EXISTS TRANSACTION_MODIFIED(
    ...
    FOREIGN KEY (TRANSID) REFERENCES TRANSACTION_HISORY(ID) ON DELETE CASCADE 
);
...
INSERT INTO TRANSACTION_HISORY(ACCOUNTID,AMOUNT,DATE) 
    SELECT ID,AMOUNT,DATE FROM ACCOUNT; 
...
CREATE VIEW ALL_TRANS AS
    SELECT B.ID AS B_ID,A.ID AS A_ID,T.ID,T.DATE,T.AMOUNT FROM TRANSACTION_HISORY T JOIN ACCOUNT A ON A.ID =T.ACCOUNTID JOIN ASSOCIATED ON  A.ID = ASSOCIATED.TYPE JOIN BANK_ACCOUNT AS B ON ASSOCIATED.BANK_ACCOUNTID = B.ID;